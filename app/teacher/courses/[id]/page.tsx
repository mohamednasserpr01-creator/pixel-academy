...
const initialData: Lecture[] = [
    { id: 'lec-1', title: 'المحاضرة الأولى: أساسيات', items: [{ id: 'item-1', type: 'lesson', title: 'شرح', viewsLimit: 3 }] }
];
...
export default function CourseBuilderPage({ params }: { params: Promise<{ id: string }> }) {
    ...
    // 🚀 State بيانات الكورس مع إضافة إعدادات النشر والصلاحية والغلق
    ...
    return (
        <div style={{ animation: 'fadeIn 0.4s ease', maxWidth: '1000px', margin: '0 auto', paddingBottom: '50px' }}>
            ...
            <div>
                {/* 🚀 محتوى تاب الإعدادات الأساسية الجديد بالكامل */}
                {activeTab === 'settings' && (
                    <div style={{ animation: 'fadeIn 0.3s ease', display: 'flex', flexDirection: 'column', gap: '20px' }}>
                        ...
                        <div style={{ background: 'var(--card)', padding: '25px', borderRadius: '15px', border: '1px solid rgba(255,255,255,0.05)' }}>
                            ...
                            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px', marginBottom: '20px' }}>
                                ...
                                <div style={{ gridColumn: '1 / -1' }}>
                                    <label style={{ display: 'block', color: 'var(--txt-mut)', marginBottom: '8px', fontSize: '0.9rem', fontWeight: 'bold' }}>وصف الكورس (يظهر للطلاب)</label>
                            ...
                            <div>
                                <label style={{ display: 'block', color: 'var(--txt-mut)', marginBottom: '8px', fontSize: '0.9rem', fontWeight: 'bold' }}>صورة غلاف الكورس (Thumbnail)</label>
                                <label style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: '10px', background: 'rgba(0,0,0,0.2)', border: '2px dashed rgba(255,255,255,0.2)', padding: '30px', borderRadius: '15px', cursor: 'pointer', color: 'var(--txt-mut)' }}>
                                    <FaImage size={30} />
                        ...
                        {/* 🚀 إعدادات النشر والصلاحية */}
                        <div style={{ background: 'var(--card)', padding: '25px', borderRadius: '15px', border: '1px solid rgba(255,255,255,0.05)' }}>
                            <h3 style={{ margin: '0 0 20px 0', color: 'white', display: 'flex', alignItems: 'center', gap: '10px' }}>
                                <FaClock color="#f1c40f"/> إعدادات النشر والصلاحية
                            </h3>
                            ...
                            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px' }}>
                                <div>
                                    ...
                                    <input 
                                        type="datetime-local" 
                                        value={courseMeta.publishDate} 
                                        onChange={e => setCourseMeta({...courseMeta, publishDate: e.target.value})}
                                        style={{ width: '100%', padding: '12px 15px', background: 'rgba(0,0,0,0.3)', border: '1px solid rgba(255,255,255,0.1)', color: 'white', borderRadius: '8px', outline: 'none' }} 
                                    />
                                    <span style={{ fontSize: '0.8rem', color: 'var(--p-purple)' }}>* اتركه فارغاً ليكون متاحاً فوراً.</span>
                                </div>
                                ...
                                <div>
                                    ...
                                    <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                                        <input 
                                            type="number" min={0} 
                                            value={courseMeta.expireAfterDays} 
                                            onChange={e => setCourseMeta({...courseMeta, expireAfterDays: Number(e.target.value)})}
                                            style={{ width: '100px', padding: '12px 15px', background: 'rgba(0,0,0,0.3)', border: '1px solid rgba(255,255,255,0.1)', color: 'white', borderRadius: '8px', outline: 'none', textAlign: 'center' }} 
                                        />
                                        <span style={{ color: 'var(--txt-mut)' }}>يوم</span>
                                    </div>
                                    <span style={{ fontSize: '0.8rem', color: '#f1c40f' }}>* اكتب 0 ليكون الكورس متاحاً مدى الحياة.</span>
                                </div>
                            </div>
                            ...
                            <div style={{ marginTop: '25px', display: 'flex', flexDirection: 'column', gap: '15px', borderTop: '1px dashed rgba(255,255,255,0.1)', paddingTop: '20px' }}>
                                <label style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: 'rgba(0,0,0,0.2)', padding: '15px', borderRadius: '10px', cursor: 'pointer' }}>
                                    <div>
                                        <div style={{ color: 'white', fontWeight: 'bold', marginBottom: '5px' }}>إيقاف بيع الكورس (للمشتركين فقط)</div>
                                        <div style={{ color: 'var(--txt-mut)', fontSize: '0.85rem' }}>لن يتمكن أي طالب جديد من الشراء، وسيظل الكورس متاحاً لمن اشتراه مسبقاً.</div>
                                    </div>
                                ...
                                <label style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: courseMeta.lockForAll ? 'rgba(231, 76, 60, 0.1)' : 'rgba(0,0,0,0.2)', border: courseMeta.lockForAll ? '1px solid #e74c3c' : '1px solid transparent', padding: '15px', borderRadius: '10px', cursor: 'pointer', transition: '0.3s' }}>
                                    <div>
                                        <div style={{ color: courseMeta.lockForAll ? '#e74c3c' : 'white', fontWeight: 'bold', marginBottom: '5px' }}>إغلاق الكورس تماماً (على جميع الطلاب)</div>
                                        <div style={{ color: 'var(--txt-mut)', fontSize: '0.85rem' }}>سيتم منع الجميع من الدخول إليه، حتى من قام بشرائه.</div>
                                    </div>
                    ...
                    <div style={{ animation: 'fadeIn 0.3s ease' }}>
                        ...
                        <DndContext sensors={sensors} collisionDetection={closestCenter} onDragEnd={handleDragEnd}>
                            {curriculum.map((lecture: Lecture) => (
                                <LectureCard key={lecture.id} lecture={lecture} onUpdateLecture={updateLecture} onOpenContentPicker={(lecId: string) => setActiveLectureId(lecId)} onOpenSettings={(item: LectureItemType) => setSettingsItem({ lectureId: lecture.id, item })} onOpenReports={(item: LectureItemType) => setReportItem(item)} onOpenLectureReports={(lec: Lecture) => setReportLecture(lec)} onOpenGrading={(item: LectureItemType) => setGradingItem(item)} />
                            ))}
                        </DndContext>
                    </div>
                )}
