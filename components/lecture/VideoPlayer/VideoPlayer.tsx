...
export default function VideoPlayer({ activeItem, studentName }: { activeItem: any, studentName: string }) {
    ...
    useEffect(() => {
        if (videoRef.current) {
            setIsPlaying(false); setProgress(0); setCurrentTimeStr("00:00");
            videoRef.current.load();
        }
    }, [activeItem]);

    const formatTime = (sec: number) => { let m=Math.floor(sec/60), s=Math.floor(sec%60); return `${m}:${s<10?'0'+s:s}`; };
    const togglePlay = () => { if(videoRef.current) { videoRef.current.paused ? videoRef.current.play() : videoRef.current.pause(); setIsPlaying(!videoRef.current.paused); } };
    ...
    const toggleFullScreen = () => { if(!playerContainerRef.current) return; !document.fullscreenElement ? playerContainerRef.current.requestFullscreen() : document.exitFullscreen(); };
    ...
    return (
        <div className={`pixel-player ${!isPlaying ? 'paused' : ''}`} ref={playerContainerRef} onContextMenu={e => e.preventDefault()}>
            ...
            <video ref={videoRef} src={activeItem.videoSrc} poster={activeItem.poster} onTimeUpdate={handleTimeUpdate} onLoadedMetadata={() => { if(videoRef.current) setDurationStr(formatTime(videoRef.current.duration)); }} playsInline></video>
                ...
                <div className="controls-row">
                    ...
                    <div className="controls-right">
                        <div className="speed-menu">
                            <button className="ctrl-btn" onClick={(e) => { e.stopPropagation(); setShowSpeedMenu(!showSpeedMenu); }}><FaTachometerAlt /></button>
